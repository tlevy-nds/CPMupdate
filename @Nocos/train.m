function [vals, keepInds, Bs] = train(obj, X, Y, los, predictorNames, imputeMethod, resamplingMethod, nfeatures, lambda, useCV, likelihoodDist, controlRandomNumberGeneration, coefficientAnalysis, usePiecewise)
...
% obj.mus = reshape(obj.mus, [], 1);
% obj.sigs = reshape(obj.sigs, [], 1);
...
[B, fitinfo] = lasso(zs(trainingInds, :), Y_(trainingInds), 'Alpha', 1, 'Standardize', false, ...
    ...
    % I wanted to see the variability of running this multiple times
    % Bootstrapping helps me better estimate the distributions
    B = Bs(:, end);
%     myFields = fieldnames(fitinfo);
%     for ifield = 1:length()
%         fitinfo.(myFields{ifield}) = median(fitinfo.(myFields{ifield}));
%     end
end
...
% [~, si] = sort(abs(B), 'descend');  % This was what I had originally
[~, si] = sort(abs(B(:, ind)), 'descend');

% obj.nfeatures is an upper bound
nfeatures = min(nfeatures, nnz(B));
inds = si(1:nfeatures);

% conform to format of other models
try
    obj.coefs = [fitinfo.Intercept(ind) - sum(obj.mus .* B ./ obj.sigs); B ./ obj.sigs];
catch
end
...
if isempty(vals)
    vals = lasso_reconstruct(zs(:, inds), obj.lassoCoeffs, obj.biasTerm, nfeatures);
else
    % plot the cross-validation result
    if true
        obj.estimate_distributions(vals, Y_, los, likelihoodDist, usePiecewise);
